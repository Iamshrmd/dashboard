<template>
  <section class="relative">
    <aside @click.self="closeSideBar()" class="sidebar overflow-y-auto no-scrollbar transition-all duration-500 bg-[#171821] w-[88px] h-screen py-6 flex flex-col gap-y-10 items-center border-r border-[#2c2d33]"  :class="{'!w-[160px]':sideBarWidth}">
        ...
        <div class="flex flex-col gap-y-5 h-full w-full px-4 relative">
            <div @click="showdashitem(i,icon.title)" @mouseenter="toggleIcon(i)" v-for="(icon,i) in iconImgs" :data-id="i" :key="i" class="iconDiv hover:cursor-pointer w-full h-9 flex items-center justify-center rounded-md">
                <a class="flex items-center" href="#">
                    ...
                    <img class="w-4 h-4" :src="`${icon.blackImg}`" :class="{'hidden' : !icon.imgVisible}">
                </a>
